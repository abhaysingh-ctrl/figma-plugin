...
the holistic UX review (typography, spacing, icon style, components, information
architecture, accessibility/WCAG, UX laws, copy vs. brand voice) runs there —
those checks need LLM judgment and web search, so they can't run inside Figma's
sandbox. The plugin's only job is to build the correct node-scoped share link and
the prompt text so the hand-off is one click instead of "copy link to selection"
plus typing.

For Amino design-system token compliance instead, use the sibling `amino-help`
plugin.
